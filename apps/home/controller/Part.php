...
class Part extends Indexbase {
    ...
    public function index(){
        ...
        if(!empty($template['list']) && isset($template['list']) && is_file(config('template.view_path').$template['list'])){
            $temps = config('template.view_path').$template['list'];
        }elseif(!empty($parts['plate']) && isset($parts['plate'])){
...
//            俊先专用
            if($parts['id'] == '62'){
                $chinalist = $chinacode->allcode();
                $this->assign('chinacode',$chinalist);
            }
        }
        //  查询上级栏目
        ...
        if(!empty($parts['seotitle']) && isset($parts['seotitle'])){
            $webs['title'] = $parts['seotitle'];
        }
        //  俊先专用
        if(!empty($data['chc']) && isset($data['chc'])){
            $diquname = $chinacode->where('zoneid',$data['chc'])->find();
        ...
        switch ($data['class']){
            case 0:
                ...
                if(!empty($partlist)){
                    ...
                    $fids = implode(',',$dangqian);
                }
                //  获取大分类模板文件
                $tempdir = $this->temp.'largepart/';
                ...
                //  获取内容
                $artlist = $pages = '';
                if(!empty($fids) && isset($fids)){
                    $artlist = $artmodel->where('fid','in',$fids)->where('status','1')->order($order)->paginate($data['maxpage']);
