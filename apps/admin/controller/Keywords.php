...
use app\common\model\Keywords as cxModel;
use think\Loader;
...
class Keywords extends Adminbase {
    ...
    public function add(){
        ...
        if(request()->isPost()){
            $data = input('post.');
            $data = datatrim($data);
            $validate = Loader::validate('Keywords');
            if(!$validate->scene('add')->check($data)){
                $this->error($validate->getError());
            }
            $data['uid'] = $this->cxbsuser['uid'];
            if($cxmodel->allowField(true)->save($data)){
                $this->addlog('添加【'.$data['title'].'】关键词成功');
                $this->success('添加【'.$data['title'].'】关键词成功','index');
            }else{
                $this->error('添加【'.$data['title'].'】关键词失败');
            }
        }
        return view();
    }
    public function edit(){
        ...
        if(request()->isPost()){
            ...
            if(model('Keywords')->allowField(true)->save($data,$data['id'])){
                $this->addlog('修改【'.$data['title'].'】关键词成功');
                $this->success('修改【'.$data['title'].'】关键词成功','index');
            }else{
                $this->error('修改【'.$data['title'].'】关键词失败');
            }
        }
    ...
    public function del(){
        ...
        $title = $deldata['title'];
        if($deldata->delete()){
            $this->addlog('删除【'.$title.'】关键词成功');
            $this->success('删除【'.$title.'】关键词成功','index');
        }else{
            $this->error('删除【'.$title.'】关键词失败');
        }
        return view();
    }
